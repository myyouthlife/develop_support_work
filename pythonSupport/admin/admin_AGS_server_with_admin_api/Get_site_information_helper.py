...
class ADMINself(object):
    ...
    def addPermission(self,serviceName,roleName,isAllowed=True):
        ''' update:20151130
        serviceName format=<folder>/

        '''
        if str.lower(serviceName)=='public':
            current_permission=self.sendAGSReq(self.URL+"/services/{0}/permissions/add".format(serviceName)+self.basicQ,{"principal":"esriEveryone",
          "isAllowed":True})
            print current_permission
        else:
            current_permission=self.sendAGSReq(self.URL+"/services/{0}/permissions".format(serviceName)+self.basicQ,'')
            for i in current_permission['permissions']:
                if i['principal']=='esriEveryone':
                     current_permission=self.sendAGSReq(self.URL+"/services/{0}/permissions/add".format(serviceName)+self.basicQ,{"principal":"esriEveryone",
              "isAllowed":False})
            current_permission2=self.sendAGSReq(self.URL+"/services/{0}/permissions/add".format(serviceName)+self.basicQ,{"principal":roleName,
              "isAllowed":isAllowed})
            print current_permission2
